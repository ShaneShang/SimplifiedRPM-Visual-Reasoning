...
np.random.seed(seed)
start_epoch, total_epochs, print_epoch = 0, 150, 5

gen_id = gen_id_list_pre[i_split]
save_name = 'SCL_split'+str(i_split)

save_dir, log_file = setup_savename(save_name)
loader_train, loader_val_list, loader_gen_list = load_data(gen_id)

############### model ############### 
model = SCL_supervised().to(device)
...
for current_epoch in range(start_epoch, total_epochs+1):
    ...
    if current_epoch % print_epoch == 0:
        ...
        feature_gen = get_feature(model, loader_gen_list, device, False)[0]
    ...
    if current_epoch < warmup_epoch: 
        loss_train = train_epoch(model, loader_train, optimizer, device, warmup_lr[current_epoch])
    else: 
        loss_train = train_epoch(model, loader_train, optimizer, device, None)
